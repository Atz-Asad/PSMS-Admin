...
if(isset($_POST['change_btn'])){
    ...
    if(empty($current_password)){
        ...
        $error = "New Password & Current Password is does't match!";
    }
    else if(SHA1($current_password) != $db_password){
        $error = "Current Password is Wrong!";
    }
    else{       
        $new_password = SHA1($confirm_new_password);
        $stm=$pdo->prepare("UPDATE teachers SET password=? WHERE id=?");
        $stm->execute(array($new_password,$teacher_id));
        $success= "Password change Succcessfully";
